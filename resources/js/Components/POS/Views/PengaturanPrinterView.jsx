...
export default function PengaturanPrinterView() {
    ...
    return (
        <div className="flex-1 bg-[#f4f6f9] p-5 text-xs font-semibold text-gray-600">
            <div className="max-w-2xl bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-5">

                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-[10px] text-gray-400 font-bold uppercase">Ukuran Gulungan Kertas</label>
                        <select value={printerConfig.jenis} onChange={(e) => setPrinterConfig({...printerConfig, jenis: e.target.value})} className="w-full bg-white border border-gray-200 px-3 py-2 rounded-xl font-bold text-gray-800 focus:outline-none">
                    ...
                    <div className="space-y-1">
                        ...
                        <select value={printerConfig.koneksi} onChange={(e) => setPrinterConfig({...printerConfig, koneksi: e.target.value})} className="w-full bg-white border border-gray-200 px-3 py-2 rounded-xl font-bold text-gray-800 focus:outline-none">
                ...
                <button onClick={handleScan} disabled={isScanning} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl uppercase text-[11px] transition">
                ...
                <div className="space-y-2">
                    ...
                    <div className="border border-gray-100 bg-gray-50 rounded-xl divide-y divide-gray-100 overflow-hidden">
                        {printers.map(p => (
                            <div key={p.id} className="p-3 bg-white flex justify-between items-center">
                                ...
                                <button onClick={() => setActivePrinter(p)} className="bg-gray-100 hover:bg-emerald-600 hover:text-white font-bold px-3 py-1.5 rounded-lg transition">
                                    Sambungkan
                                </button>
                            </div>
                        ))}
