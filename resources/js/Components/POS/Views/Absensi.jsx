        ...
        <div className="flex-1 flex flex-col h-full bg-[#f8fafc] overflow-hidden relative">
            ...
            <div className="flex-1 overflow-hidden p-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div className="lg:col-span-1 flex flex-col space-y-4">
                    <div className="bg-white border border-slate-200/60 rounded-l p-5 shadow-sm relative overflow-hidden flex-shrink-0">
                        ...
                        <form onSubmit={handleAbsenMasuk} className="space-y-4">
                            <div>
                                ...
                                <div className="relative">
                                    <select 
                                        ...
                                        className="w-full bg-slate-50 border border-slate-200 rounded-l px-3 py-2.5 text-xs font-semibold text-slate-700 focus:outline-none focus:border-[#009664] transition appearance-none"
                                    >
                            ...
                            <button 
                                type="submit"
                                className="w-full bg-[#009664] text-white py-2.5 rounded-l font-black text-xs uppercase tracking-wider shadow-sm shadow-emerald-600/10 hover:bg-emerald-700 active:scale-[0.99] transition flex items-center justify-center space-x-2"
                            >
                ...
                <div className="lg:col-span-2 flex flex-col bg-white border border-slate-200/60 rounded-l shadow-sm overflow-hidden">
                    <div className="px-4 py-3 bg-slate-50/50 border-b border-slate-100 flex justify-between items-center">
                        <h3 className="font-black text-xs text-slate-700 uppercase tracking-wider flex items-center">
                            <BellElectric size={16} className="mr-2 text-[#009664]" /> Pergerakan Aktivitas Kerja Hari Ini
                        </h3>
                    ...
                    <div className="flex-1 overflow-auto">
                        {dataAbsensi.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400">
                                <ArchiveRestore size={40} className="mb-2 text-slate-400" />
                                            ...
                                            <td className="py-3 px-4 text-center">
                                                {row.status === 'Aktif Bekerja' ? (
                                                    <button
                                                        ...
                                                        className="bg-white border border-orange-200 text-orange-600 px-3 py-1 rounded-l font-bold text-[10px] hover:bg-orange-50 active:scale-95 shadow-sm transition"
                                                    >
                                                         Pulang
                                                    </button>
                                                ) : (
            ...
            {modalConfig.isOpen && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 animate-fade-in">
                    ...
                    {/* Kotak Konten Modal */}
                    <div className="bg-white rounded-l max-w-sm w-full p-5 shadow-2xl border border-slate-100 relative z-10 text-center transform scale-100 transition-transform">
                        {/* Ikon Header Modal */}
                            ...
                            {modalConfig.type === 'confirm' ? (
                                <>
                                    <button
                                        ...
                                        className="flex-1 bg-slate-100 text-slate-600 hover:bg-slate-200 font-bold text-xs py-2 px-4 rounded-l transition uppercase tracking-wider"
                                    >
                                        Batal
                                    </button>
                                    <button
                                        ...
                                        className="flex-1 bg-[#009664] text-white hover:bg-emerald-700 font-bold text-xs py-2 px-4 rounded-l transition uppercase tracking-wider shadow-sm shadow-emerald-600/10"
                                    >
                                        Ya, Pulang
                                    </button>
                                </>
                                ...
                                <button
                                    type="button"
                                    onClick={modalConfig.onConfirm}
                                    className="w-full bg-slate-800 text-white hover:bg-slate-900 font-bold text-xs py-2.5 rounded-l transition uppercase tracking-wider"
                                >
                                    Mengerti
                                </button>
                            )}
