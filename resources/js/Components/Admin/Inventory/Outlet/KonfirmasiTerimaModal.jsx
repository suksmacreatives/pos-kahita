...
export default function KonfirmasiTerimaModal({
    isOpen,
    onClose,
    data,
    onConfirm,
    ...
    const seed =
        data && data.items
            ? JSON.stringify(data.items.map((i) => i.id || i.produk_id))
            : "";
    ...
    const totalQtyKirim = (data?.items || []).reduce(
        (acc, it) => acc + (it.qty_kirim || it.qty || 0),
        0,
    );
    const totalQtyTerima = itemsState.reduce(
        (acc, it) => acc + (it.qty_terima || 0),
        0,
    );
    ...
    return createPortal(
        ...
        >
            <div
                className="relative bg-white rounded-2xl shadow-xl border border-gray-100 max-w-4xl w-full flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200"
                ...
                >
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3 text-blue-700">
                        ...
                        <div className="text-xs space-y-1 font-medium">
                            <p>
                                Periksa kembali fisik barang yang dikirim oleh
                                gudang pusat. Anda bisa menyesuaikan jumlah unit
                                jika terdapat ketidaksesuaian/kerusakan.
                            </p>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-100 text-xs text-gray-700">
                                {itemsState.map((item, idx) => (
                                    ...
                                    >
                                        <td className="px-4 py-3 font-bold text-gray-800 align-middle">
                                            {item.nama}
                                        </td>

                                        {/* Ukuran Kolom dengan Kotak Border */}
                                        ...
                                        {/* WARNA TANPA HEX - Murni Teks Badge */}
                                        <td className="px-4 py-3 align-middle">
                                            <span
                                                className={`px-1.5 py-0.5 rounded text-[9px] font-bold border ${
                                                    item.warna
                                        ...
                                        <td className="px-4 py-3 text-right font-bold text-gray-500 align-middle">
                                            {item.qty_kirim} pcs
                                        </td>
                        ...
                        <div className="bg-rose-50 border border-rose-100 rounded-xl p-4 flex gap-3 text-rose-700">
                            ...
                            <div className="text-xs space-y-1">
                                <p className="font-bold">
                                    Perbedaan Selisih Stok Terdeteksi!
                                </p>
                                <p className="font-medium">
                                    ...
                                    tidak diterima/rusak. Item selisih ini akan
                                    diretur otomatis kembali ke gudang pusat
                                    setelah konfirmasi disimpan.
                                </p>
                            </div>
                        </div>
                    )}
