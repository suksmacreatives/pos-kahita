...
export default function FormOpnameOutletModal({ open, onClose, onSubmit, products = [], processing = false }) {
  ...
  const selectedProducts = useMemo(() =>
    products.filter(p => selectedIds.includes(p.id)), [selectedIds, products]
  );
                ...
                {filtered.map(p => {
                  ...
                  return (
                    <button key={p.id} type="button" onClick={() => toggleProduct(p.id)} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs transition-colors cursor-pointer ${isSelected ? 'bg-emerald-50 border border-emerald-200' : 'border border-transparent hover:bg-gray-50'}`}>
                      ...
                      <span className="text-gray-400">{p.varian.length} varian &bull; {p.total_stok} stok</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4 p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-emerald-600">{results.length} produk &bull; Total Sistem: <strong>{grandTotalSistem}</strong> &bull; Total Aktual: <strong>{grandTotalAktual}</strong> &bull; Selisih: <strong className={grandTotalSelisih > 0 ? 'text-emerald-700' : grandTotalSelisih < 0 ? 'text-rose-600' : ''}>{grandTotalSelisih > 0 ? `+${grandTotalSelisih}` : grandTotalSelisih}</strong></p>
                </div>
              </div>
