    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b flex items-center justify-between bg-gray-50">
          <div>
            <h3 className="text-sm font-bold text-gray-800">Detail Penerimaan Barang</h3>
            <p className="text-[10px] text-gray-400">PO: {data.nomor_po} — {data.supplier_nama}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div className="grid grid-cols-3 gap-4 text-xs">
            ...
            <div><span className="font-semibold text-gray-500">Tgl PO</span><p className="font-bold text-gray-800 mt-0.5">{data.tanggal_po}</p></div>
            <div><span className="font-semibold text-gray-500">Tgl Terima</span><p className="font-bold text-gray-800 mt-0.5">{data.tanggal_terima || '-'}</p></div>
          </div>
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right font-semibold text-gray-500">Qty Pesan</th>
                  <th className="px-3 py-2 text-right font-semibold text-gray-500">Qty Terima</th>
                  <th className="px-3 py-2 text-right font-semibold text-gray-500">Harga</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {(data.items || []).map((it, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-right text-gray-700">{it.qty_pesan}</td>
                    <td className="px-3 py-2 text-right font-bold text-gray-800">{it.qty_terima}</td>
                    <td className="px-3 py-2 text-right text-gray-600">{it.harga_beli?.toLocaleString()}</td>
                    <td className="px-3 py-2 text-right font-bold text-gray-800">{(it.qty_terima * it.harga_beli).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-gray-50 border-t border-gray-200">
                <tr>
                  ...
                  <td className="px-3 py-2 text-right font-bold text-gray-900">{data.total_nilai?.toLocaleString()}</td>
                </tr>
              </tfoot>
            </table>
          </div>
