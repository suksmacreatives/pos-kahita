...
import { Shirt, Layers, Sparkles, Wind, Gem, Moon, Eye, Edit, Trash, ArrowUpDown } from 'lucide-react';
import ProductBadge from './ProductBadge';

export const categoryConfig = {
  'Atasan': { bg: 'bg-emerald-100 text-emerald-700 border-emerald-200', icon: Shirt },
  ...
  'Outer': { bg: 'bg-purple-100 text-purple-700 border-purple-200', icon: Wind },
  'Gamis & Hijab': { bg: 'bg-amber-100 text-amber-700 border-amber-200', icon: Moon },
...
export default function ProductTable({ products, onOpenDrawer, onOpenEdit, onDeleteProduct }) {
  ...
  const sortedProducts = [...products].sort((a, b) => {
    ...
    // Handle nested or processed fields for sorting
    if (sortField === 'harga_jual') {
      const aFinalPrice = a.harga_jual - (a.harga_jual * (a.diskon || 0)) / 100;
      const bFinalPrice = b.harga_jual - (b.harga_jual * (b.diskon || 0)) / 100;
      aValue = aFinalPrice;
      bValue = bFinalPrice;
    }
      ...
      <table className="w-full text-xs text-left text-gray-600 border-collapse">
        <thead className="bg-slate-50/75 border-b border-gray-100 text-gray-500 font-semibold sticky top-0 backdrop-blur-md z-10">
          <tr>
            ...
            <th className="py-3 px-4 min-w-[200px] cursor-pointer hover:bg-gray-100 transition-colors" onClick={() => handleSort('nama_produk')}>
              <div className="flex items-center gap-1.5">
                Nama & Kode
                <ArrowUpDown className="w-3.5 h-3.5 text-gray-400" />
              </div>
            </th>
            <th className="py-3 px-4 cursor-pointer hover:bg-gray-100 transition-colors" onClick={() => handleSort('kategori')}>
              <div className="flex items-center gap-1.5">
                Kategori
                <ArrowUpDown className="w-3.5 h-3.5 text-gray-400" />
              </div>
            </th>
            <th className="py-3 px-4 cursor-pointer hover:bg-gray-100 transition-colors" onClick={() => handleSort('harga_jual')}>
              <div className="flex items-center gap-1.5">
                Harga Jual
                <ArrowUpDown className="w-3.5 h-3.5 text-gray-400" />
              </div>
            </th>
            <th className="py-3 px-4 cursor-pointer hover:bg-gray-100 transition-colors" onClick={() => handleSort('total_stok')}>
              <div className="flex items-center gap-1.5">
                Stok
                <ArrowUpDown className="w-3.5 h-3.5 text-gray-400" />
              </div>
            ...
            <th className="py-3 px-4 cursor-pointer hover:bg-gray-100 transition-colors" onClick={() => handleSort('status')}>
              <div className="flex items-center gap-1.5">
                Status
                <ArrowUpDown className="w-3.5 h-3.5 text-gray-400" />
              </div>
        ...
        <tbody className="divide-y divide-gray-50">
          {sortedProducts.length === 0 ? (
            ...
            sortedProducts.map((product, index) => {
              ...
              const maxStockCapacity = 100; // standard base for percentage representation
              ...
              return (
                <tr key={product.id} className="hover:bg-slate-50/50 transition-colors group odd:bg-white even:bg-slate-50/20">
                  <td className="py-3 px-4 text-center font-medium text-gray-400">
                    {index + 1}
                  </td>
                  <td className="py-3 px-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shadow-sm ${config.bg}`}>
                  ...
                  <td className="py-3 px-4">
                    <div className="space-y-1">
                      ...
                      <div className="text-[10px] text-gray-400 pl-1">{product.sub_kategori}</div>
                    </div>
                  </td>
                  <td className="py-3 px-4">
                    {isDiscounted ? (
                      <div className="flex flex-col">
                        ...
                        <span className="text-emerald-600 font-bold">
                          ...
                          <span className="ml-1 text-[9px] bg-red-50 text-red-500 px-1 py-0.2 rounded font-semibold border border-red-100">
                            -{product.diskon}%
                          </span>
                        </span>
                      </div>
                  ...
                  <td className="py-3 px-4">
                    <div className="flex flex-col gap-1.5 max-w-[80px]">
                      <div className="flex items-center justify-between font-semibold text-gray-800">
                        <span>{product.total_stok} pcs</span>
                      </div>
